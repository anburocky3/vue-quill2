...
```
# name supports fuzzy match. will build all packages with name containing "vue-quill2":
npm run assets:build -- vue-quill2
```
*/
;(async () => {
  ...
  if (target === '') {
    logger.warning(
      '',
      'You must specify the target e.g. npm run assets:dev -- vue-quill2'
    )
    return
  }
